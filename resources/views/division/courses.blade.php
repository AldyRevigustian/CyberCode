                        ...
                        <table class="table" id="table1">
                            ...
                            <tbody>
                                @foreach ($courses as $key => $course)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td><img src={{ asset('storage/image/photo_member/' . $course->photo) }} alt=""
                                        ...
                                        <td>
                                            <a href="#" class="btn icon btn-success" data-bs-toggle="modal"
                                                data-bs-target="#modalEdit{{ $course->id }}"><i
                                                    class="bi bi-pencil-fill"></i></a>

                                            <form action="{{ route('delete.member', $course->id) }}" class="d-inline"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
