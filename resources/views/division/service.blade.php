                        ...
                        <table class="table" id="table1">
                            ...
                            <tbody>
                                @foreach ($services as $key => $service)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td><img src={{ asset('storage/image/photo_member/' . $service->photo) }} alt=""
                                        ...
                                        <td>
                                            <a href="#" class="btn icon btn-success" data-bs-toggle="modal"
                                                data-bs-target="#modalEdit{{ $service->id }}"><i
                                                    class="bi bi-pencil-fill"></i></a>

                                            <form action="{{ route('delete.member', $service->id) }}" class="d-inline"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
